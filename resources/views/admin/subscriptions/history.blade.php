        ...
        <table class="min-w-full text-sm text-gray-700">
            <thead class="bg-gray-100 text-gray-700 sticky top-0 z-10">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left font-semibold">Original Amount</th>
            ...
            <tbody>
                @forelse($payments as $payment)
                    <tr class="border-b border-gray-100 hover:bg-gray-50 transition">
                        ...
                        <td class="px-6 py-4 text-gray-700 text-green-600 font-semibold">
                            {{ number_format($payment->original_amount, 2) }}
                        </td>

                        <td class="px-6 py-4">
                            <span class="px-2 py-1 py-1 text-xs font-semibold bg-green-100 text-green-700 rounded-full">
    ...
    <div class="grid gap-4 md:hidden">
        @forelse($payments as $payment)
            <div class="bg-white border border-gray-200 shadow-sm rounded-xl p-4 space-y-3">
                <h3 class="font-semibold text-lg text-gray-900">{{ $payment->user->name }}</h3>

                <p class="text-sm text-gray-700">
                    Amount: <span class="font-medium">{{ number_format($payment->amount, 2) }}</span>
                </p>

                <p class="text-sm text-gray-700">
                    Currency: <span class="font-medium">{{ $payment->original_currency }}</span>
                </p>

                <p class="text-sm text-gray-700">
                    Original Amount: <span class="font-medium text-green-600 font-semibold">{{ number_format($payment->original_amount, 2) }}</span>
                </p>
