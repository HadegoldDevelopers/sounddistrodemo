...
<div class="w-full max-w-full lg:max-w-5xl mx-auto px-4 py-6 space-y-6">
    ...
    <div>
        <h2 class="text-3xl font-bold text-gray-900 tracking-tight">Privacy Policy</h2>
        <p class="text-sm text-gray-600 mt-1">Edit the privacy policy displayed to users.</p>
    </div>

    <form id="privacyForm" method="POST" action="{{ route('admin.settings.privacy.update') }}">
        @csrf
        ...
        <div class="bg-white shadow-sm rounded-xl border border-gray-200 p-6 space-y-4">
            ...
            <div>
                ...
                <textarea id="privacy_policy" name="privacy_policy" class="hidden">{{ old('privacy_policy', $privacyPolicy) }}</textarea>
                <div x-ignore>
    <div id="privacy_editor" class="min-h-[500px]"></div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const quill = new Quill('#privacy_editor', {
        theme: 'snow',
        modules: {
            toolbar: [
                [{ 'header': [1, 2, 3, false] }],
